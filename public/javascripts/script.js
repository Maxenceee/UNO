...
var Socket = function(g, n) {
	...
	try {
		...
		let WSProtocol = (location.protocol === 'https:') ? 'wss:' : 'ws:',
			...
			spath = WSProtocol.concat(WSHost, (g.noop ? '?no_op' : ''));
...
g.onFinish = function(a) {
	...
	window.setTimeout(() => {
		...
		t.oppn.forEach(e => e.gtc().resize(S, T).moveTo(t.pileCoords, 0));
...
g.updateOpponentDeckAndGamepack = function(a) {
	...
	if (a.deckSize > this.oppn.length) {
		...
		let size = this.oppn.length - a.deckSize;
		for(var i = 0; i < size; i++) {
			let del = this.oppn.shift();
			let u = () => {
				this.gamepack.update(a.card || this.gamepack.pack.cardCode, a.newColor || this.gamepack.customColor);
				del.delete();
			}
			del.gtc()
				.resize(S, T)
